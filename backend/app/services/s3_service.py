...
def generate_presigned_upload_url(
    ...
    try:
        ...
        raise StorageError(f"Could not create upload link: {exc}")
...
def generate_presigned_download_url(key: str) -> str:
    ...
    try:
        ...
        raise StorageError(f"Could not create download link: {exc}")
...
def get_object_content(key: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error("Failed to read S3 object %s: %s", key, exc)
        raise StorageError(f"Failed to read file: {exc}")
...
def get_object_bytes(key: str) -> Optional[bytes]:
    ...
    try:
        ...
        logger.error("Failed to read S3 object bytes %s: %s", key, exc)
        raise StorageError(f"Failed to read file: {exc}")
...
def delete_object(key: str) -> None:
    ...
    try:
        ...
        raise StorageError(f"Failed to delete file: {exc}")
...
def copy_to_kb_bucket(source_key: str, dest_key: str | None = None) -> str:
    ...
    try:
        ...
        raise StorageError(f"Failed to copy to KB bucket: {exc}")
...
def put_object_to_kb_bucket(
    ...
    try:
        ...
        raise StorageError(f"Failed to write to KB bucket: {exc}")
