        ...
        return (
          <div className="p-4 rounded-xl bg-white border border-slate-200 shadow-sm space-y-3 animate-fade-in">
            ...
            {j.key_guidelines?.length > 0 && (
              <div>
                <div className="text-[11px] text-slate-500 font-medium mb-1">📋 Applicable Guidelines</div>
                <div className="flex flex-wrap gap-1.5">
                  {j.key_guidelines.map((g, i) => (
                    <span key={i} className="text-[11px] px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 border border-slate-200/60">{g}</span>
                  ))}
                </div>
              </div>
            )}
