  ...
  const handleBenchmark = async () => {
    ...
    try {
      ...
      const msg = err?.response?.status === 404
        ? 'Please analyze your protocol first before running benchmarks.'
        : err?.response?.status === 503 || err?.message?.includes('timeout')
        ? 'ClinicalTrials.gov may be temporarily unavailable. Please try again in a minute.'
        : 'Benchmarking could not be completed. Please try again.';
      setError(msg);
    } finally {
    ...
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-xl bg-brand-50/60 border border-brand-100/60">
        ...
        <p className="text-[11px] text-slate-600 leading-relaxed">
          ...
          Results combine <span className="font-semibold">live ClinicalTrials.gov data</span> with
          <span className="font-semibold"> AI analysis grounded on ICH/FDA/EMA guidelines</span> from the Knowledge Base.
          Each metric links to the relevant regulatory guideline for verification.
        </p>
      </div>
      ...
      {benchmark && (
        <div className="space-y-3">
                      ...
                      <div className="mt-1 pt-2 border-t border-slate-100 space-y-2 animate-fade-in">
                        ...
                        {b.guideline_ref && (
                          <div className="p-2 rounded-lg bg-blue-50 border border-blue-100">
                            <div className="text-[10px] text-blue-600 font-semibold mb-0.5">📋 Regulatory Reference</div>
                            <p className="text-[11px] text-blue-700">{b.guideline_ref}</p>
                          </div>
                        )}
                      </div>
                    )}
          ...
          {benchmark.regulatory_benchmarks?.length > 0 && (
            <div className="space-y-2">
              <div className="text-[11px] text-slate-500 uppercase tracking-wider font-medium">
                Regulatory Compliance Benchmarks
              </div>
              {benchmark.regulatory_benchmarks.map((rb, i) => (
                <div key={i} className="p-3 rounded-xl bg-white border border-slate-200 shadow-sm">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium text-slate-800">{rb.requirement}</span>
                    <span className={cn(
                      'text-[11px] px-1.5 py-0.5 rounded font-medium border',
                      rb.protocol_status === 'compliant' ? 'text-emerald-700 bg-emerald-50 border-emerald-200' :
                      rb.protocol_status === 'partial' ? 'text-amber-700 bg-amber-50 border-amber-200' :
                  ...
                  {rb.recommendation && (
                    <p className="text-[11px] text-slate-600">{rb.recommendation}</p>
                  )}
                  {rb.guideline_code && (
                    <span className="inline-block mt-1 text-[10px] text-blue-600 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded">
                      📋 {rb.guideline_code}
                    </span>
                  )}
                </div>
          ...
          <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200/60">
            <p className="text-[10px] text-slate-500 leading-relaxed">
              <span className="font-semibold text-slate-600">ℹ️ Data Sources:</span>{' '}
              Trial metrics sourced from <span className="font-semibold">ClinicalTrials.gov API</span> (live data).
              Assessments and regulatory benchmarks generated by AI cross-referencing
              ICH, FDA, and EMA guidelines. Expand any metric to see the relevant guideline reference.
            </p>
          </div>
        </div>
      )}
